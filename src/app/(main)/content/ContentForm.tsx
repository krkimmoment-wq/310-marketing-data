...
export default function ContentForm({ cohortId }: { cohortId?: number }) {
  ...
  const inputCls = "mt-1 w-full px-3 py-2 rounded-lg border border-slate-300 outline-none focus:border-blue-500";
  return (
    <form onSubmit={submit} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
      <div className="font-bold text-slate-700">콘텐츠 추가</div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <div className="text-sm md:col-span-3">
          <span className="text-slate-500">플랫폼 <span className="text-slate-400">(여러 개 동시 선택 — 한 게시물을 여러 채널에 올린 경우)</span></span>
        ...
        <label className="text-sm"><span className="text-slate-500">비틀리 클릭 <span className="text-slate-400">(sync가 자동 채움)</span></span>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50">취소</button>
      </div>
    </form>
  );
}
