  ...
  if (!editing) {
    return (
      <tr className="border-t border-slate-100">
        <td className="px-4 py-2 text-slate-600 whitespace-nowrap">{c.pub_date}</td>
        <td className="px-4 py-2 text-slate-700">{c.category ?? "-"}</td>
        <td className="px-4 py-2 text-slate-500 text-xs max-w-[140px] truncate" title={c.platform ?? ""}>{c.platform ?? "-"}</td>
        <td className="px-4 py-2 font-medium text-slate-800 max-w-xs truncate" title={c.title ?? ""}>{c.title ?? "-"}</td>
        <td className="px-4 py-2 text-slate-700 text-right">{(c.bitly_clicks ?? 0).toLocaleString("ko-KR")}</td>
        <td className="px-4 py-2 text-slate-500">{c.reaction || "-"}</td>
        <td className="px-4 py-2 text-slate-600 whitespace-nowrap">{c.status ?? "-"}</td>
  ...
  return (
    <tr className="border-t border-blue-200 bg-blue-50/40">
      <td colSpan={8} className="px-4 py-4">
        <div className="font-bold text-slate-700 mb-3">✏️ 콘텐츠 수정</div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <label className="text-sm"><span className="text-slate-500">발행일</span>
            <input type="date" value={form.pub_date} onChange={(e) => set("pub_date", e.target.value)} className={inputCls} /></label>
          <label className="text-sm"><span className="text-slate-500">카테고리</span>
            <select value={form.category} onChange={(e) => set("category", e.target.value)} className={inputCls}>
              {CATEGORIES.map((x) => <option key={x}>{x}</option>)}
            </select></label>
          <label className="text-sm"><span className="text-slate-500">상태</span>
            <select value={form.status} onChange={(e) => set("status", e.target.value)} className={inputCls}>
              {STATUSES.map((x) => <option key={x}>{x}</option>)}
            </select></label>
          <label className="text-sm md:col-span-3"><span className="text-slate-500">제목 / 내용</span>
            <input value={form.title} onChange={(e) => set("title", e.target.value)} className={inputCls} /></label>
          <div className="text-sm md:col-span-3">
            <span className="text-slate-500">플랫폼 (복수 선택)</span>
            <div className="mt-1 flex flex-wrap gap-2">
              {PLATFORMS.map((p) => (
                <button key={p} type="button" onClick={() => togglePlat(p)}
                  className={`px-3 py-1.5 rounded-lg border text-xs ${form.platforms.includes(p) ? "bg-blue-600 text-white border-blue-600" : "border-slate-300 text-slate-600 hover:bg-slate-50"}`}>
                  {p}
                </button>
              ))}
            </div>
          </div>
          <label className="text-sm"><span className="text-slate-500">비틀리 클릭 (sync 자동)</span>
            <input type="number" value={form.bitly_clicks} onChange={(e) => set("bitly_clicks", Number(e.target.value))} className={inputCls} /></label>
          <label className="text-sm"><span className="text-slate-500">반응 (예: 좋아요 54)</span>
            <input value={form.reaction} onChange={(e) => set("reaction", e.target.value)} className={inputCls} /></label>
          <label className="text-sm"><span className="text-slate-500">도달 (선택)</span>
            <input value={form.reach} onChange={(e) => set("reach", e.target.value)} className={inputCls} /></label>
          <label className="text-sm md:col-span-3"><span className="text-slate-500">비고 (선택)</span>
