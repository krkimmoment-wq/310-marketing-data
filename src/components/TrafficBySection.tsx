...
export default function TrafficBySectionView({ data }: { data: NonNullable<TrafficBySection> }) {
  ...
  return (
    <div className="jarvis-card p-5">
      ...
      <div className="text-[11px] text-slate-500 mb-4">구간별 채널 조회 <b className="text-slate-300">총량</b>(화력)과 소스 믹스(질)를 지난 기수와 비교. 같은 분기점 구간끼리라 기간이 동일(일수 병기). 빨강 = {aName} 화력 부족 구간.</div>

      <div className="divide-y divide-slate-800">
        {sections.map((sec) => {
          // 같은 분기점 구간끼리는 일수가 같음 — 구간 총량으로 비교
